...
async function signUp(req, res) {
    ...
    const { accountType, password } = account;
    ...
    try {
      ...
      const newAccount = new Account({
        user: newUser._id,
        accountIdentifier: email, // Using email as accountIdentifier
        accountType,
...
const login = async (req, res) => {
    try {
      const { accountIdentifier,email , password } = req.body;
  
      const account = await Account.findOne({
          $or: [{ accountIdentifier }, { email }]
        });
    
      if (!account) {
         return res.status(401).json({ error: 'No account found with this identifier or email' });
      }
