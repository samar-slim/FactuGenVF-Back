...
async function upload(req, res) {
    try {
        ...
        if (req.file.mimetype === 'application/pdf'){
            ...
            // Execute with retry and wait for it to finish before continuing
            let counter = 0 
            let MaxRetries = 3
            while (true){ 
                try{
...
async function executeWithRetry(callback, retries = 3, ...agrs) {
    for (let i = 0; i < retries; i++) {
        try {
            await callback(...agrs);
            console.log("Success");

            break; // Exit the loop if the callback succeeds
        } catch (error) {
